...
class MultilingualNanoBEIREvaluator(NanoBEIREvaluatorST):
    ...
    def _write_csv(self, output_path, epoch, steps, agg_results):
        ...
        with open(csv_path, mode="a", encoding="utf-8") as f:
            ...
            for k in self.accuracy_at_k:
                output_data.append(agg_results.get(f"{self.name}_MaxSim_accuracy@{k}", ""))
            for k in self.precision_recall_at_k:
                output_data.append(agg_results.get(f"{self.name}_MaxSim_precision@{k}", ""))
                output_data.append(agg_results.get(f"{self.name}_MaxSim_recall@{k}", ""))
            for k in self.mrr_at_k:
    ...
    def _write_per_language_csv(self, output_path, epoch, steps, per_lang_results):
        ...
        csv_path = os.path.join(
            output_path, f"MultilingualNanoBEIR_PyLate_{self.aggregate_key}_per_language.csv",
        )
        ...
        with open(csv_path, mode="a", encoding="utf-8") as f:
            for lang in self.languages:
                ...
                for k in self.accuracy_at_k:
                    output_data.append(per_lang_results.get(f"{self.name}_{lang}_MaxSim_accuracy@{k}", ""))
                for k in self.precision_recall_at_k:
                    output_data.append(per_lang_results.get(f"{self.name}_{lang}_MaxSim_precision@{k}", ""))
                    output_data.append(per_lang_results.get(f"{self.name}_{lang}_MaxSim_recall@{k}", ""))
                for k in self.mrr_at_k:
                    output_data.append(per_lang_results.get(f"{self.name}_{lang}_MaxSim_mrr@{k}", ""))
                for k in self.ndcg_at_k:
                    output_data.append(per_lang_results.get(f"{self.name}_{lang}_MaxSim_ndcg@{k}", ""))
                for k in self.map_at_k:
                    output_data.append(per_lang_results.get(f"{self.name}_{lang}_MaxSim_map@{k}", ""))
                f.write(",".join(map(str, output_data)) + "\n")
    ...
    def _log_per_language_results(self, per_lang_results):
        ...
        for lang in self.languages:
            ...
            for k in self.accuracy_at_k:
                ...
                logger.info(f"    Accuracy@{k}: {val * 100:.2f}%")
            for k in self.precision_recall_at_k:
                prec = per_lang_results.get(f"{self.name}_{lang}_MaxSim_precision@{k}", 0)
